...
// InitDatasets extracts the filenames from downloader bucket
...
func Annotate(w http.ResponseWriter, r *http.Request) {
	...
	result, err := cache.Annotate(data)
	if err != nil {
...
func BatchAnnotate(w http.ResponseWriter, r *http.Request) {
	...
	for _, data := range dataSlice {
		responseMap[data.IP+strconv.FormatInt(data.Timestamp.Unix(), encodingBase)], err = cache.Annotate(&data)
		if err != nil {
